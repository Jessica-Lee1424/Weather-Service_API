...
class HistoryService {
  ...
  private async write(cities: City[]): Promise<void> {
    return new Promise((resolve, reject) => {
        ...
        fs.writeFile('db/searchHistory.json', jsonData, 'utf8', (err) => {
            if (err) {
                reject(err);
            } else {
                resolve();
...
async addCity(cityName: string, lat: number, lon: number): Promise<void> {
  try {
      ...
      const newCity = new City(newId, cityName, lat, lon); // Create a new City object
...
historyService.addCity('Chicago', 40.7128, -74.0060)
.then(() => {
